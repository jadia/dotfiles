...
# Clean the monitor setup
# rm -rf /tmp/monitor_setup
#declare -a modes=( extended hdmi-only lcd-only mirrored )
declare -a modes=( dual extended lcd-only )
...
function switch_setup {
	case "$@" in
		...
		;;
		"dual" )
			xrandr --auto && xrandr --output $LEFT_DISPLAY --primary --left-of $RIGHT_DISPLAY
			# xrandr --auto && xrandr --output $SECOND_DISPLAY --primary --above $LAPTOP_DISPLAY
